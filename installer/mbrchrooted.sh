...
sed -i "s@GRUB_CMDLINE_LINUX_DEFAULT=\"logleveel=3 quiet\"@GRUB_CMDLINE_LINUX_DEFAULT=\"rd.luks.uuid=${cryptrot} rd.luks.options=discard root=/dev/mapper/vg0-root loglevel=3 quiet rd.udev.log_priority=3 rd.systemd.show_status=auto fan_control=1\"@" /etc/default/grub
